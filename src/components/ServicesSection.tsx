...
const ServicesSection = () => {
  ...
  const handleServiceClick = (serviceName: string) => {
    toast(`Просмотр услуги`, {
      description: `Вы выбрали услугу: ${serviceName}`,
    });
  };

  return (
    <section className=" bg-white dark:bg-navy-dark transition-colors duration-300">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 pb-[30px] gap-2">
          {/* Service Cards */}
          <Animable index={1} className={'p-4 bg-gray-50 dark:bg-navy/50 rounded-xl transition-all duration-300 hover:shadow-xl'}>
            <div className="group  animate-fade-up">
              ...
              <Link to="/services/offers" onClick={() => handleServiceClick("Специальные предложения")}>
                <Button variant="ghost" className="text-navy-dark dark:text-white group-hover:text-blue-600">
                  {t('learn_more_about')} <ArrowRight className=" w-auto h-4 transition-transform group-hover:translate-x-1" />
                </Button>
              </Link>
            </div>
          </Animable>
          <Animable index={2} className={'p-4 bg-gray-50 dark:bg-navy/50 rounded-xl transition-all duration-300 hover:shadow-xl'}>
          ...
          <Animable index={3} className={'p-4 bg-gray-50 dark:bg-navy/50 rounded-xl transition-all duration-300 hover:shadow-xl'}>
          <div className="group animate-fade-up">
